...
Automated Metadata Tracking: Maintains a metadata table to track which PDF files have been processed and which are pending.
Date Extraction: Parses date information directly from source PDF filenames to generate target file names.
Layered Data Architecture:
Bronze Layer: Raw data loaded as-is from source files for auditability.
Silver Layer: Data cleansing and standardization (e.g., converting NULL values to -1), with data quality expectations. Bad records are isolated into a separate Silver error table.
